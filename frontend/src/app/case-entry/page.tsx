...
export default function CaseEntryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post("https://healthpulse-4d9z.onrender.com/api/encounters/finalize/api/clinical/analyze", {
        patient_id: 1,
        narrative,
        language
      });
      ...
      alert("Error connecting to backend API (https://healthpulse-4d9z.onrender.com/api/encounters/finalize). Check if backend is running.");
      console.error(err);
    } finally {
      setLoading(false);
    }
